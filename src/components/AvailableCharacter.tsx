          ...
          <tbody>
            ...
            {sortedAvailable.map((c, idx) => {
              ...
              const otherPartyTooltipTitle = otherPartyInfos?.length
                ? `Este personagem já está ${otherPartyInfos.length > 1 ? "em outras PT's" : "em outra PT"}:\n\n${otherPartyInfos.map(info => `• ${info.name} — ${info.questLabel}${info.statusNote ? ` (${info.statusNote})` : ""}`).join("\n")}`
