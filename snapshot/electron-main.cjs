...
// ============================================================================
// PRÉ-VALIDAÇÃO DE DISPONIBILIDADE — jogadores online no topo do Bazaar
// ----------------------------------------------------------------------------
// Antes de iniciar a listagem, confirmamos que o SERVIDOR está realmente de pé
// e que o site já saiu da manutenção: o topo da página do Bazaar exibe a
// quantidade de jogadores online (ex.: <span class="TopBar-module__LYwH0W__
// onlineNumber">5129</span>). Só consideramos o site pronto quando esse número
// é >= RUBINOT_MIN_ONLINE_PLAYERS.
//
// ROBUSTEZ: o hash do CSS-module ("LYwH0W") muda a cada build do site, então o
// seletor NÃO depende dele — casamos pelo sufixo estável "onlineNumber" e, em
// último caso, por qualquer elemento curto do topo cujo texto contenha
// "online" + um número. Valor ausente, vazio ou não numérico NUNCA conta como
// servidor disponível (fail-closed).
// ============================================================================
const RUBINOT_MIN_ONLINE_PLAYERS = 1000;
const RUBINOT_AVAILABILITY_RETRY_MS = 60 * 1000;      // Auto-Bazaar: nova verificação a cada 1 minuto.
const RUBINOT_AVAILABILITY_MAX_WAIT_MS = 6 * 60 * 60 * 1000; // Teto de segurança do laço automático.

/** Lê o número de jogadores online exibido na página do Bazaar. Nunca lança (exceto navegador fechado). */
async function readRubinotOnlineCount(page) {
  try {
    const result = await page.evaluate(() => {
      const parseNum = (text) => {
        const digits = String(text || '').replace(/[^\d]/g, '');
        if (!digits || digits.length > 9) return NaN;
        return Number(digits);
      };
      // 1) Seletor preferido: classe de CSS-module com sufixo estável
      //    "onlineNumber" (o hash do meio varia por build — não dependemos dele).
      const direct = document.querySelector('[class*="onlineNumber" i]');
      if (direct) {
        const count = parseNum(direct.textContent);
        if (Number.isFinite(count)) return { found: true, count, source: 'onlineNumber-class' };
      }
      // 2) Fallback estrutural: elemento curto cujo texto combine "online" com
      //    um número (ex.: "5129 Online"). Textos longos são ignorados para não
      //    casar com parágrafos que apenas mencionem a palavra.
      const candidates = Array.from(document.querySelectorAll('a, span, div, li'));
      for (const el of candidates) {
        const text = String(el.textContent || '').trim();
        if (!text || text.length > 40) continue;
        if (!/online/i.test(text)) continue;
        if (!/\d/.test(text)) continue;
        const count = parseNum(text);
        if (Number.isFinite(count)) return { found: true, count, source: 'online-text' };
      }
      return { found: false, count: NaN, source: '' };
    });
    if (result && result.found && Number.isFinite(result.count) && result.count >= 0) return result;
    return { found: false, count: NaN, source: String(result?.source || '') };
  } catch (error) {
    if (isRubinotBrowserClosedError(error)) throw error;
    return { found: false, count: NaN, source: '', error: String(error?.message || error) };
  }
}
 ...
 * número de online não for encontrado ou estiver abaixo do mínimo, retorna
 * `ok:false` com mensagem clara — a consulta é abortada e o Boss é informado
 * no Painel Bazaar. Nada da consulta atual é executado.
 *
 * AUTO-BAZAAR (autoRun=true): espera inteligente. Permanece na página,
 * recarrega a cada 1 minuto e repete a verificação até o site estar pronto —
 * então devolve a MESMA sessão validada que o fluxo normal usaria, e a
 * consulta prossegue sozinha, sem qualquer ação manual. O botão "Concluir
 * agora" continua funcionando durante a espera (parada cooperativa).
 */
async function ensureRubinotServerAvailability(progressSender, context, { autoRun = false } = {}) {
  const label = 'bazaar-availability';
  const startedAt = Date.now();
  let attempt = 0;
  ...
  for (;;) {
    attempt += 1;
    if (isRubinotManualStopRequested()) {
      return { ok: false, stoppedManually: true, error: 'Consulta encerrada pelo Boss durante a verificação de disponibilidade do Bazaar.' };
    }

    // A MESMA validação de sessão do fluxo atual (Cloudflare + DOM do Bazaar +
    // API da listagem respondendo JSON). Nenhuma etapa nova aqui — apenas a
    // reutilizamos como parte da verificação de disponibilidade.
    const session = await ensureRubinotSessionReady(context, null, attempt === 1 ? 'bazaar-session' : `${label}-retry-${attempt}`);
    lastSession = session;
    ...
    rubinotDiag(label, 'Verificação de disponibilidade do servidor.', {
      attempt,
      autoRun,
      sessionOk: !!session.ok,
      onlineFound: !!online.found,
      onlineCount: online.found ? online.count : null,
      onlineSource: online.source || '',
      minimo: RUBINOT_MIN_ONLINE_PLAYERS,
      pronto: onlineReady,
      elapsedMs: Date.now() - startedAt,
    });

    if (onlineReady) {
      // Servidor disponível: a consulta prossegue EXATAMENTE como hoje, com a
      // sessão validada (inclusive `apiData` da página 1, que evita repetição).
      return { ok: true, session, onlineCount: online.count };
    }

    // Diagnóstico legível do que impediu a liberação.
    const detail = !session.ok
      ? (session.message || 'a página do Bazaar não terminou de carregar (site possivelmente em manutenção)')
      : !online.found
        ? 'o número de jogadores online não foi encontrado na página (site possivelmente em manutenção ou ainda não atualizado)'
        : `apenas ${online.count} jogador(es) online — mínimo exigido: ${RUBINOT_MIN_ONLINE_PLAYERS}`;

    if (!autoRun) {
      // CONSULTA MANUAL: aborta e informa o Boss no Painel Bazaar.
      return {
        ok: false,
        serverNotReady: true,
        onlineCount: online.found ? online.count : null,
        needsHumanVerification: !!session.needsHumanVerification,
        error: `O servidor/site do Bazaar ainda não está pronto para a consulta: ${detail}. Aguarde a atualização do Bazaar e consulte novamente.`,
      };
    }

    // AUTO-BAZAAR: teto de segurança para o laço nunca ficar preso para sempre
    // (ex.: manutenção estendida de um dia inteiro).
    if (Date.now() - startedAt >= RUBINOT_AVAILABILITY_MAX_WAIT_MS) {
      return {
        ok: false,
        serverNotReady: true,
        onlineCount: online.found ? online.count : null,
        error: `Auto-Bazaar: o servidor não ficou disponível após ${Math.round(RUBINOT_AVAILABILITY_MAX_WAIT_MS / 3600000)}h de espera (${detail}). A consulta foi encerrada; ela será refeita no próximo ciclo automático.`,
      };
    }

    // Espera de 1 minuto com verificação do "Concluir agora" a cada segundo e
    // progresso visível no Painel Bazaar (contagem regressiva).
    for (let waitedMs = 0; waitedMs < RUBINOT_AVAILABILITY_RETRY_MS; waitedMs += 1000) {
      if (isRubinotManualStopRequested()) {
        return { ok: false, stoppedManually: true, error: 'Consulta encerrada pelo Boss durante a espera pela disponibilidade do Bazaar.' };
      }
      if (waitedMs % 15000 === 0) {
        const remainingS = Math.max(1, Math.round((RUBINOT_AVAILABILITY_RETRY_MS - waitedMs) / 1000));
        sendRubinotProgress(progressSender, buildRubinotProgress('bazaar', `Auto-Bazaar aguardando o servidor: ${detail}. Nova verificação em ${remainingS}s...`, 0, 0));
      }
      await sleep(1000);
    }

    // Recarrega a página para forçar uma reavaliação real do site (a próxima
    // iteração revalida sessão + DOM + API + jogadores online do zero).
    try {
      const page = (session && session.page) ? session.page : await getRubinotSessionPage(context);
      sendRubinotProgress(progressSender, buildRubinotProgress('bazaar', 'Auto-Bazaar: atualizando a página do Bazaar para nova verificação...', 0, 0));
      await page.reload({ waitUntil: 'domcontentloaded', timeout: 60000 }).catch(error => {
        if (isRubinotBrowserClosedError(error)) throw error;
        rubinotDiag(label, 'Falha tolerada ao recarregar a página na espera do Auto-Bazaar.', { error: String(error?.message || error) });
      });
    } catch (error) {
      if (isRubinotBrowserClosedError(error)) throw error;
      rubinotDiag(label, 'Falha tolerada ao obter a página para recarga.', { error: String(error?.message || error) });
    }
  }
}

async function fetchRubinotBazaarWithPlaywright(progressSender = null, browserKey = rubinotSelectedBrowser, endUntilTs = 0, cleanProfile = false, autoRun = false) {
  ...
  rubinotFetchInFlight = (async () => {
    try {
      ...
      // ── NOVA ETAPA: disponibilidade do servidor ANTES da consulta ─────────
      // Engloba a validação de sessão que já existia (mesma função, mesma
      // ordem) e acrescenta a checagem de jogadores online >= 1000. Manual:
      // aborta e informa o Boss. Auto-Bazaar: espera com recarga a cada 1 min
      // até liberar, então segue naturalmente para o fluxo atual.
      const readiness = await ensureRubinotServerAvailability(progressSender, context, { autoRun });
      if (!readiness.ok) {
        return {
          ok: false,
          error: readiness.error || 'Não foi possível confirmar que o Bazaar está disponível/atualizado.',
          needsHumanVerification: !!readiness.needsHumanVerification,
          serverNotReady: !!readiness.serverNotReady,
          onlineCount: Number.isFinite(readiness.onlineCount) ? readiness.onlineCount : null,
          cancelled: !!readiness.stoppedManually,
          auctions: [],
          total: 0,
          fetchedAt: Date.now(),
        };
      ...
      rubinotDiag('bazaar', 'Servidor disponível: prosseguindo com a consulta normal.', {
        jogadoresOnline: readiness.onlineCount,
        minimo: RUBINOT_MIN_ONLINE_PLAYERS,
      });
...
ipcMain.handle('rubinot-bazaar-fetch', async (event, payload = {}) => {
  ...
  // Consulta automática (Auto-Bazaar): a pré-validação de disponibilidade
  // entra em modo de espera inteligente (recarga a cada 1 min) em vez de
  // abortar. Consulta manual mantém o aborto imediato com aviso ao Boss.
  ...
  return runRubinotQueued('bazaar-fetch', () => fetchRubinotBazaarWithPlaywright(event.sender, rubinotSelectedBrowser, endUntilTs, rubinotUseCleanProfile, autoRun));
});
