...
const firebaseConfig = {
  ...
  // Realtime Database — usado EXCLUSIVAMENTE pela presença de usuários online
  // (status/{uid}/conns). OPCIONAL: sem esta variável no .env, `rtdb` fica
  // null e o app degrada para "presence indisponível" sem quebrar nada.
...
if (hasValidConfig) {
  try {
    ...
    // RTDB da presença: só conecta com databaseURL configurada e fora do modo
    // Emulator (a presença não participa dos testes locais de Firestore).
    if (firebaseConfig.databaseURL && !useFirebaseEmulators) {
      try {
        rtdb = getDatabase(app);
      } catch (rtdbError) {
        console.warn("Realtime Database indisponível (presença desativada):", rtdbError);
        rtdb = null;
      }
    }
...
export { auth, db, rtdb, isSimulationMode };
export {
  onSnapshot,
  setDoc,
  updateDoc,
