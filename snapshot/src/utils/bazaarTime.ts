...
// ============================================================================
// HELPERS DE DATA/HORA DO BAZAAR (fonte única)
// ----------------------------------------------------------------------------
// Movidos do BazarPanel.tsx para cá SEM alteração de comportamento, para que
// o painel "Personagens com Itens" reutilize exatamente as mesmas regras de
// fuso e formatação — requisito da funcionalidade (nenhuma lógica duplicada).
// ============================================================================

/** Normaliza o fim do leilão para SEGUNDOS (aceita ms ou s). */
export function normalizeAuctionEndTimestamp(ts: number | null): number | null {
...
/** Converte "YYYY-MM-DDTHH:MM" no fuso configurado para timestamp UNIX (s). */
export function parseDateTimeLocalWithOffset(value: string, offsetMinutes: number): number {
...
export function getBazarEndUntilAtConfiguredTime(offsetMinutes: number, dayOffsetFromToday: number, hour: number, minute = 0, nowMs = Date.now()): string {
...
/** Padrão do "Encerra até": hoje/amanhã às 10:00, conforme a hora atual. */
export function getDefaultBazarEndUntil(offsetMinutes: number): string {
