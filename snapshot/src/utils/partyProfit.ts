...
/**
 * ARREDONDAMENTO DA DIVISÃO (múltiplo de 25 com corte em 10).
 *
 * Mesma definição de `roundSplitTo25` em `PartyPanel`: para baixo quando
 * faltam MAIS de 10 unidades para o múltiplo de 25 superior; para cima quando
 * faltam 10 ou menos. Ex.: 389 -> 375 · 390 -> 400.
 * Duplicada aqui pelo mesmo motivo de `floorTo25Profit` (módulo puro).
 */
export function roundSplitTo25Profit(value: number): number {
  if (!Number.isFinite(value) || value <= 0) return 0;
...
export function getSplitValuePerMember(party: PartyTab): number {
  ...
  return roundSplitTo25Profit(getSplitTotal(party) / members.length);
}
