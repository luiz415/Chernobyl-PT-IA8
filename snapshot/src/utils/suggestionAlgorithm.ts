...
import { characterQuestEligible, serviceQuestEligible, collectBusyIdsForQuest, type QuestFilter } from "./questEligibility";
...
function collectEligibleCandidates(
  ...
  // 1. Determinar IDs ocupados em outras PTs ativas — POR QUEST.
  // Só ocupa quem está em PT cuja Quest é a MESMA da sugestão: um personagem
  // em PT de Sanguine continua elegível para sugestão de Soul War (e
  // vice-versa). Ver `collectBusyIdsForQuest` (fonte única da regra).
  const busyIds = collectBusyIdsForQuest(allParties, options.questType, currentPartyId);
