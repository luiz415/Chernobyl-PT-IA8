...
/**
 * CANCELAMENTO DA PRÉ-VENDA — exclusivo do DONO original (ou Boss), e somente
 * enquanto o comprador ainda não confirmou a compra (`status: pre_approved`).
 *
 * O reset é um DELETE do documento compartilhado: o ID é determinístico
 * (dono+personagem) e o `create` da pré-aprovação exige documento inexistente,
 * então apagar é a única forma de devolver o personagem ao fluxo normal de
 * negociação. A validação roda DENTRO da transação: se o comprador confirmar
 * o pagamento em paralelo, o status já não é `pre_approved` e o cancelamento
 * é negado — nunca cancela uma compra confirmada.
 */
export async function cancelCharacterAcquisitionPreApproval(id: string, actorUid?: string, actorRole?: string): Promise<{ ok: boolean; error?: string }> {
  ...
  const validate = (current: CharacterAcquisition): string | null => {
    if (actor !== current.originalOwnerUid && actorRole !== "Boss") {
      return "Somente o dono original pode cancelar esta pré-venda.";
    }
    if (normalizeCharacterAcquisitionStatus(current.status) !== "pre_approved") {
      return "A pré-venda não pode mais ser cancelada: o comprador já confirmou a compra.";
    }
    return null;
  };
  ...
  if (isSimulationMode || !db) {
    ...
    const error = validate(records[index]);
  ...
  try {
    ...
    await runTransaction(db, async transaction => {
      const snapshot = await transaction.get(ref);
      if (!snapshot.exists()) {
        cancelError = "Negociação não encontrada.";
        return;
      }
    ...
    return { ok: false, error: error?.message || "Não foi possível cancelar a pré-venda." };
  }
}
