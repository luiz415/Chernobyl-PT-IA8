...
interface RankingPanelProps {
  ...
  /**
   * MODO DEMONSTRATIVO DO TUTORIAL: quadro fictício exibido no lugar dos
   * dados reais. Quando presente, NENHUMA leitura do Firestore acontece
   * (carga inicial, paginação, posição exata e reset ficam desativados).
   */
  demoEntries?: RankingEntry[];
...
export default function RankingPanel({
  ...
  // MODO DEMO (tutorial): quadro fictício, zero leituras do Firestore.
  const demoMode = !!demoEntries;
  ...
  // loading — nenhum efeito de leitura roda (guardas `demoMode` abaixo).
  useEffect(() => {
    if (!demoMode || !demoEntries) return;
    // Modo consolidado forçado: as entradas fictícias já representam o mês
    // corrente e não devem passar pelo recorte legado de dailyStats.
    monthlyModeRef.current = true;
  ...
  useEffect(() => {
    if (demoMode) return; // demo: sem releitura
  ...
  const handleLoadMore = async () => {
    if (demoMode) return; // demo: quadro fixo, sem paginação real
  ...
  useEffect(() => {
    if (demoMode) { setMyExactPosition(null); return; } // demo: sem consulta
