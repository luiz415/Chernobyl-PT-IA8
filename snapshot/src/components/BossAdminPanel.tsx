...
export default function BossAdminPanel({ open, onClose, presenceMap = {}, minAverage = 10, globalSettings, pendingDonationsCount: externalPendingDonationsCount = 0, pendingRequestsCount: externalPendingRequestsCount = 0, pendingVipCount: externalPendingVipCount = 0 }: Props) {
  ...
  // ── Filtros + ordenação da tabela de Usuários ─────────────────────────────
  // Substituem a antiga barra de pesquisa geral: cada coluna filtrável tem o
  // seu controle na linha logo abaixo do cabeçalho, e todos combinam entre si.
  const [userFilterText, setUserFilterText] = useState("");            // Usuários (nome OU e-mail)
  const [userFilterStatus, setUserFilterStatus] = useState("");        // Status (aprovado/recusado)
  const [userFilterPresence, setUserFilterPresence] = useState("");    // Visto por último (On/Off)
  const [userFilterMedia, setUserFilterMedia] = useState("");          // Média Doação (número)
  const [userFilterMediaOp, setUserFilterMediaOp] = useState<"gte" | "lte">("gte");
  const [userSortKey, setUserSortKey] = useState<"nome" | "status" | "presence" | "media" | null>(null);
  ...
  // ── Valores derivados por usuário (mesma lógica das células) ──────────────
  // Centralizados aqui para filtro, ordenação e exibição usarem EXATAMENTE o
  // mesmo número/estado — nenhuma regra nova, apenas a extração do que as
  // células já calculavam inline.
  const getUserMediaDonation = (uid: string): number => {
    ...
    return totalPtsConcluidas > 0 ? Math.round(totalRcAprovado / totalPtsConcluidas) : 0;
  };
  ...
  const filteredActiveUsers = useMemo(() => {
    ...
    const mediaTarget = userFilterMedia !== "" ? parseInt(userFilterMedia, 10) : null;
    const rows = activeUsers.filter(u => {
      // Usuários: nome OU e-mail (equivalente à antiga pesquisa geral)
      if (text && !u.nome.toLowerCase().includes(text) && !u.email.toLowerCase().includes(text)) return false;
      // Status
      if (userFilterStatus && u.status !== userFilterStatus) return false;
      // Visto por último: On/Off
      if (userFilterPresence === "On" && !getUserOnline(u.uid)) return false;
      if (userFilterPresence === "Off" && getUserOnline(u.uid)) return false;
      // Média Doação (>= ou <=)
      if (mediaTarget !== null && Number.isFinite(mediaTarget)) {
        const media = getUserMediaDonation(u.uid);
        if (userFilterMediaOp === "gte" && media < mediaTarget) return false;
        if (userFilterMediaOp === "lte" && media > mediaTarget) return false;
      }
      return true;
    });
    ...
    const sorted = [...rows].sort((a, b) => {
      ...
      switch (userSortKey) {
        ...
        case "status":
          ...
          break;
        case "presence": {
          // Online primeiro; empate decidido pela atividade mais recente
          const aOn = getUserOnline(a.uid) ? 1 : 0;
        ...
        case "media":
          cmp = getUserMediaDonation(a.uid) - getUserMediaDonation(b.uid);
          break;
      }
  ...
  }, [activeUsers, userFilterText, userFilterStatus, userFilterPresence, userFilterMedia, userFilterMediaOp, userSortKey, userSortDir, userStatsMap, presenceMap, globalSettings?.presenceEnabled]);

  function toggleUserSort(key: "nome" | "status" | "presence" | "media") {
    if (userSortKey !== key) { setUserSortKey(key); setUserSortDir("asc"); }
    else if (userSortDir === "asc") setUserSortDir("desc");
    else { setUserSortKey(null); setUserSortDir("asc"); }
  }
  function UserSortIcon({ col }: { col: "nome" | "status" | "presence" | "media" }) {
    if (userSortKey !== col) return <ArrowUpDown size={11} className="opacity-30" />;
    return userSortDir === "asc" ? <ArrowUp size={11} className="text-emerald-400" /> : <ArrowDown size={11} className="text-emerald-400" />;
  }
  ...
  return (
    ...
    >
      <div className="app-modal-frame app-modal-size-xl app-modal-frame--scroll bg-[var(--th-n-elev)] border border-white/10 rounded-2xl shadow-2xl w-full max-w-4xl" style={{ height: "min(85dvh, 58rem)" }}>
        ...
        {/* Tab Selection — guias compactas em flex-wrap, sem scroll horizontal */}
          ...
          <div className="flex flex-wrap gap-1">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex-1 min-w-[104px] px-2 py-1.5 text-[11px] font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer whitespace-nowrap ${
                  activeTab === tab.key
        ...
        <div className="app-modal-body p-4 sm:p-5">
          ...
          {activeTab === "users" && (
            <div className="flex flex-col h-full gap-2.5">
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-1.5 text-[10px] text-amber-300 flex items-center gap-2 flex-shrink-0">
              ...
              <div className="flex-1 min-h-0 overflow-y-auto border border-white/5 rounded-xl bg-[var(--th-n-panel)]">
                <table className="w-full border-collapse text-left text-xs">
                  <thead className="sticky top-0 z-10">
                    {/* Cabeçalho ordenável — "Ações" fica de fora (comandos) */}
                    <tr className="bg-[var(--th-n-hi)] border-b border-white/5">
                      <th className="px-3 py-2 font-bold uppercase tracking-wider text-slate-400 cursor-pointer hover:text-slate-200 select-none transition-colors" onClick={() => toggleUserSort("nome")}>
                        <div className="flex items-center gap-1">Usuários <UserSortIcon col="nome" /></div>
                      </th>
                      <th className="px-3 py-2 font-bold uppercase tracking-wider text-slate-400 cursor-pointer hover:text-slate-200 select-none transition-colors" onClick={() => toggleUserSort("status")}>
                        <div className="flex items-center gap-1">Status <UserSortIcon col="status" /></div>
                      </th>
                      <th className="px-3 py-2 font-bold uppercase tracking-wider text-slate-400 cursor-pointer hover:text-slate-200 select-none transition-colors" onClick={() => toggleUserSort("presence")}>
                        <div className="flex items-center gap-1">Visto por último <UserSortIcon col="presence" /></div>
                      </th>
                      <th className="px-3 py-2 font-bold uppercase tracking-wider text-slate-400 text-center cursor-pointer hover:text-slate-200 select-none transition-colors" onClick={() => toggleUserSort("media")}>
                        <div className="flex items-center justify-center gap-1">Média Doação <UserSortIcon col="media" /></div>
                      </th>
                      <th className="px-3 py-2 font-bold uppercase tracking-wider text-slate-400 text-right">Ações</th>
                    </tr>
                    {/* Linha de filtros — combinam entre si; Ações não filtra */}
                    <tr className="bg-[var(--th-n-panel)] border-b border-white/10">
                      <th className="px-3 py-1.5 font-normal">
                        <FilterInline value={userFilterText} onChange={setUserFilterText} placeholder="Nome ou e-mail" maxWidth="170px" />
                      </th>
                      <th className="px-3 py-1.5 font-normal">
                        <FilterSelect label="Status" options={["aprovado", "recusado"]} selected={userFilterStatus} onSelect={setUserFilterStatus} allLabel="Todos" placeholder="Status" />
                      </th>
                      <th className="px-3 py-1.5 font-normal">
                        <FilterSelect label="Visto por último" options={["On", "Off"]} selected={userFilterPresence} onSelect={setUserFilterPresence} allLabel="Todos" placeholder="On/Off" />
                      </th>
                      <th className="px-3 py-1.5 font-normal">
                        <div className="flex justify-center">
                          <FilterNumber
                            label="Média Doação (RC/PT)"
                            placeholder="RC/PT"
                            value={userFilterMedia !== "" ? parseInt(userFilterMedia, 10) : null}
                            operator={userFilterMediaOp}
                            onChange={(v, op) => { setUserFilterMedia(v === null ? "" : String(v)); setUserFilterMediaOp(op); }}
                          />
                        </div>
                      </th>
                      <th className="px-3 py-1.5 font-normal text-right">
                        <button
                          type="button"
                          onClick={() => { setUserFilterText(""); setUserFilterStatus(""); setUserFilterPresence(""); setUserFilterMedia(""); setUserFilterMediaOp("gte"); setUserSortKey(null); setUserSortDir("asc"); }}
                          className="px-2 py-1 rounded-md bg-white/5 hover:bg-white/10 border border-white/10 text-slate-400 hover:text-white text-[9px] font-bold uppercase tracking-wider transition-colors cursor-pointer"
                          title="Limpar todos os filtros e a ordenação"
                        >
                          ↺ Limpar
                        </button>
                      </th>
                  ...
                  <tbody>
                    {filteredActiveUsers.length === 0 ? (
                      <tr><td colSpan={5} className="text-center py-10 text-slate-500 italic">Nenhum usuário corresponde aos filtros atuais.</td></tr>
                    ) : (
                      filteredActiveUsers.map((user, rowIdx) => (
                        <tr key={user.uid} className={`border-b border-white/5 transition-colors hover:bg-white/[0.03] ${rowIdx % 2 === 0 ? "bg-transparent" : "bg-white/[0.015]"}`}>
                          ...
                          <td className="px-3 py-2 text-center">
                            {(() => {
                              const media = getUserMediaDonation(user.uid);
