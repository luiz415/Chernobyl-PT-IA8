...
interface FilterNumberProps {
  ...
  disabled?: boolean;
  /**
   * Aceita números NEGATIVOS (ex.: filtro de Potencial: ≥ -20). Opt-in —
   * com o default (false) o comportamento continua idêntico ao histórico
   * (apenas dígitos), sem mudança para os demais filtros numéricos.
   */
...
function FilterNumber({
  ...
  // RASCUNHO DE TEXTO do input: necessário para NEGATIVOS — o "-" sozinho é
  // um estado de digitação válido ("-" → "-2" → "-20") que ainda não tem
  // número correspondente. Com o input controlado direto por `value`
  // (número), o "-" virava null e o campo era esvaziado a cada tecla,
  // impedindo digitar o sinal. O rascunho preserva a digitação; `value`
  // continua sendo a única fonte do FILTRO aplicado.
  const [draft, setDraft] = useState<string>(value !== null ? String(value) : "");

  // Sincroniza o rascunho quando o VALOR muda por fora (limpar filtro,
  // restaurar persistido): só sobrescreve se o texto atual não representa
  // o mesmo número — não interfere com "-" em digitação.
  useEffect(() => {
    const parsedDraft = draft === "" || draft === "-" ? null : parseInt(draft, 10);
    const normalizedDraft = Number.isFinite(parsedDraft as number) ? parsedDraft : null;
    if (normalizedDraft !== value) setDraft(value !== null ? String(value) : "");
  ...
  function handleValueChange(raw: string) {
    // Negativo SOMENTE com allowNegative (opt-in): um único "-", apenas no
    // início. Sem a prop, comportamento histórico intacto (só dígitos).
          ...
          <div className="px-3 pb-2.5 pt-1">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <input
                ...
                placeholder={allowNegative ? "Digite um número (aceita negativo)..." : "Digite um número..."}
              ...
              {active && (
                <button
                  type="button"
                  onClick={() => { setDraft(""); onChange(null, operator); }}
