...
import { ArrowDown, ArrowUp, ArrowUpDown, Trash2, Pencil, Eye, EyeOff, Check, RotateCcw, ChevronLeft, ChevronRight, Plus, Shield, Handshake, Lock, X, Coins } from "lucide-react";
...
export default function CharTable({ characters, activeParties = [], readOnly, showSaleDate, onAdd, onEdit, onDelete, onToggleShare, onToggleShareAll: _onToggleShareAll, onNoteChange, onCharacterInlineChange, probableMarkers = {}, negotiatedCharacterIds = EMPTY_NEGOTIATED_CHARACTER_IDS, lockedQuestFinancialIds = EMPTY_NEGOTIATED_CHARACTER_IDS }: Props) {
  ...
  const columns: ColumnDef[] = useMemo(() => [
    ...
    },
    {
      ...
      render: (c) => {
        const locked = lockedQuestFinancialIds.has(c.id);
        // Botão "Vendido" (fluxo Item Vendido) — mesmo modal do PartyPanel.
        // Ativo apenas com DROP SW selecionado; contexto SEMPRE Soul War
        // (Lucro SW), nunca misturado com o registro de Sanguine.
        const hasDrop = !!(c.itemDropadoSW || "").trim();
        const hasSale = !!c.itemSaleSW && (c.itemSaleSW.resultRC || 0) > 0;
        const canSell = !locked && !readOnly && hasDrop && !!onCharacterInlineChange;
        return (
          <span className={`inline-flex items-center gap-1 tabular-nums text-[11px] ${locked ? "text-slate-500" : "text-slate-300"}`} title={locked ? "Bloqueado: o Lucro SW desta Quest pertence ao comprador da negociação" : undefined}>
            {locked && <Lock size={10} className="text-violet-300" />}
            {onCharacterInlineChange && !locked && !readOnly && (
              <button
                type="button"
                disabled={!canSell}
                onClick={(e) => { e.stopPropagation(); if (canSell) setItemSaleTarget({ characterId: c.id, quest: "soulwar" }); }}
                title={!hasDrop ? "Selecione um item em DROP SW para habilitar a venda" : hasSale ? "Venda registrada — clique para revisar" : "Registrar a venda do item (valor, cotação do RC e Taxa Market)"}
    ...
    },
    {
      ...
      render: (c) => {
        const locked = lockedQuestFinancialIds.has(c.id);
        // Mesmo fluxo, contexto SEMPRE Sanguine (Lucro SG) — separado de SW.
        const hasDrop = !!(c.itemDropadoSG || "").trim();
        const hasSale = !!c.itemSaleSG && (c.itemSaleSG.resultRC || 0) > 0;
        const canSell = !locked && !readOnly && hasDrop && !!onCharacterInlineChange;
        return (
          <span className={`inline-flex items-center gap-1 tabular-nums text-[11px] ${locked ? "text-slate-500" : "text-slate-300"}`} title={locked ? "Bloqueado: o Lucro SG desta Quest pertence ao comprador da negociação" : undefined}>
            {locked && <Lock size={10} className="text-violet-300" />}
            {onCharacterInlineChange && !locked && !readOnly && (
              <button
                type="button"
                disabled={!canSell}
                onClick={(e) => { e.stopPropagation(); if (canSell) setItemSaleTarget({ characterId: c.id, quest: "sanguine" }); }}
                title={!hasDrop ? "Selecione um item em DROP SG para habilitar a venda" : hasSale ? "Venda registrada — clique para revisar" : "Registrar a venda do item (valor, cotação do RC e Taxa Market)"}
                className={`inline-flex items-center gap-0.5 px-1 py-0.5 rounded border text-[9px] font-bold transition-colors ${
                  hasSale
                    ? "border-emerald-500/50 bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 cursor-pointer"
                    : canSell
  ...
  return (
    <div className="h-full flex flex-col overflow-x-scroll overflow-y-auto bg-[var(--th-bg-base)] max-w-full min-w-0 relative border-t border-[var(--th-line)]/100">
      ...
      {/* MODAL "ITEM VENDIDO" (Lucro SW/Lucro SG) — mesmo fluxo do PartyPanel.
          A gravação usa `onCharacterInlineChange`, o MESMO caminho de
          persistência do DROP SW/SG (setData -> auto-save + sharedCharacters).
          Contexto por Quest: soulwar -> itemSaleSW/dropSW; sanguine ->
          itemSaleSG/dropBakra. Nunca mistura os dois. */}
      {itemSaleTarget && (() => {
