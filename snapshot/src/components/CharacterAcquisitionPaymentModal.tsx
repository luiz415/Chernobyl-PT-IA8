...
export interface CharacterAcquisitionPaymentModalContext {
  ...
  confirmLabel: string;
  /**
   * PAGAMENTO POSTERIOR (aceite da aquisição): quando o vendedor autorizou na
   * pré-aprovação, o comprador pode marcar "Pagar somente após a venda do
   * personagem". Quando não autorizado, a opção aparece desativada.
   * Ausente (undefined) em modais que não são o aceite (ex.: repasse).
   */
...
interface Props {
  ...
  onClose: () => void;
  /** `deferPayment` só é enviado pelo aceite; demais fluxos ignoram. */
  onConfirm: (options?: { deferPayment: boolean }) => Promise<{ ok: boolean; error?: string }>;
}
...
export default function CharacterAcquisitionPaymentModal({ open, context, onClose, onConfirm }: Props) {
  ...
  // "Pagar somente após a venda do personagem" — sempre DESMARCADA ao abrir.
  ...
  // A opção de pagamento posterior só existe no ACEITE (deferredPaymentAllowed
  // definido pelo chamador). `false` = vendedor não autorizou → desativada.
  const showDeferOption = isAcquisitionAcceptance && context.deferredPaymentAllowed !== undefined;
  const deferActive = deferPayment && context.deferredPaymentAllowed === true;
  const acceptanceInstruction = deferActive
    ? `Nenhum pagamento agora: os ${formatRC(context.calculation?.total ?? context.amount)} ficam como pendência sua com ${context.recipientName}. Após a venda do personagem, uma única compensação encerra a pendência (venda maior que o devido: o vendedor paga a diferença a você; menor: você paga a diferença a ele).`
    : context.calculation
  ...
  async function submit(event: React.FormEvent) {
    ...
    // Pagamento posterior: nenhum envio agora, logo o Main Character do
    // recebedor não é um pré-requisito para registrar o aceite.
    if (!deferActive && !paymentContext.mainCharacterName.trim()) {
    ...
    setError("");
    try {
      const result = await onConfirm(showDeferOption ? { deferPayment: deferActive } : undefined);
      if (!result.ok) {
    ...
    <div className="app-modal-overlay fixed inset-0 z-[720] flex items-center justify-center bg-black/80 backdrop-blur-sm" onMouseDown={event => { if (event.target === event.currentTarget && !isSaving) onClose(); }}>
      <form onSubmit={submit} className="app-modal-frame app-modal-size-sm app-modal-frame--scroll w-full max-w-md rounded-2xl border border-emerald-500/35 bg-[var(--th-bg-base)] shadow-2xl shadow-black/70">
        ...
        <div className="app-modal-body space-y-3 p-4 sm:p-5">
          ...
          {/* PAGAMENTO POSTERIOR — só no aceite. Desativada (não selecionável)
              quando o vendedor não autorizou na pré-aprovação. */}
          {showDeferOption && (
            <label
              className={`flex items-center justify-between gap-3 rounded-lg border px-3 py-2 transition-colors ${
                context.deferredPaymentAllowed
                  ? deferPayment
                    ? "cursor-pointer border-violet-400/45 bg-violet-500/[0.1]"
                    : "cursor-pointer border-violet-500/25 bg-violet-500/[0.05] hover:bg-violet-500/[0.08]"
                  : "cursor-not-allowed border-white/10 bg-white/[0.02] opacity-55"
              }`}
              title={context.deferredPaymentAllowed
                ? "Não pagar agora: a pendência é quitada em uma única compensação após a venda do personagem"
                : "O vendedor não autorizou o pagamento após a venda nesta negociação"}
            >
              <span>
                <span className="text-xs font-bold text-slate-200">Pagar somente após a venda do personagem</span>
                <span className="block text-[9px] text-slate-500">
                  {context.deferredPaymentAllowed
                    ? "Autorizado pelo vendedor. Nenhum pagamento agora; a diferença é compensada em uma única transação após a venda."
                    : "Indisponível: o vendedor não autorizou esta modalidade."}
                </span>
              </span>
              <input
                type="checkbox"
                checked={deferPayment && context.deferredPaymentAllowed === true}
                disabled={!context.deferredPaymentAllowed}
                onChange={event => setDeferPayment(event.target.checked)}
                className="h-4 w-4 flex-shrink-0 accent-violet-500 disabled:cursor-not-allowed"
                aria-label="Pagar somente após a venda do personagem"
              />
            </label>
          )}

          <p className={`rounded-lg border px-3 py-2 text-[11px] leading-relaxed ${deferActive ? "border-violet-500/25 bg-violet-500/[0.07] text-violet-100" : "border-emerald-500/20 bg-emerald-500/[0.06] text-emerald-100"}`}>{acceptanceInstruction}</p>
