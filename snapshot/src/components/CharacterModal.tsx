...
interface Props {
  ...
  /**
   * true quando o personagem foi negociado/vendido para OUTRO usuário
   * (aquisição confirmada em que o usuário atual é o dono original).
   * Alimentado pelo App com a MESMA fonte de `negotiatedOriginalCharacterIds`
   * — o estado real de propriedade, não quem está visualizando o modal.
   * Controla o desconto adicional de 50 RC do botão "Aplicar Taxa":
   * personagem ainda do usuário → taxa % + 50 RC; negociado → só a taxa %.
   */
  negotiatedToOtherUser?: boolean;
}
...
export default function CharacterModal({ open, initial, accounts, servers, onSave, onClose, mode, highlightFields = [], lockedQuestFinancialFields = false, negotiatedToOtherUser = false }: Props) {
  ...
  // APLICAR TAXA
  //
  // 1) Taxa percentual configurada (lógica original, inalterada).
  // 2) Desconto FIXO adicional de 50 RC — SOMENTE quando o personagem ainda
  //    pertence ao usuário atual. Se foi negociado/vendido para outro usuário
  //    (prop `negotiatedToOtherUser`, derivada das aquisições confirmadas em
  //    que o usuário é o dono original — mesma fonte que trava Drop/Lucro),
  //    aplica-se apenas a taxa percentual.
  //
  // Sem acúmulo em múltiplos cliques: o botão fica desabilitado com
  // `taxApplied`, e o estado é restaurado de taxaAplicada/valorVendaOriginal
  // ao reabrir o modal — o mesmo controle que já protegia a taxa percentual
  // protege o desconto fixo.
  // ==========================================================================
  const OWNED_SALE_FLAT_FEE_RC = 50;

  function applyTax() {
    const original = data.valorVenda;
    if (original <= 0) return;
    const percentDiscounted = Math.round(original * (1 - taxPercent / 100));
    const flatFee = negotiatedToOtherUser ? 0 : OWNED_SALE_FLAT_FEE_RC;
    const discounted = Math.max(0, percentDiscounted - flatFee);
        ...
        <div className="app-modal-body p-4 sm:p-6 space-y-4">
          ...
          <div className="flex justify-end">
            <button
              ...
              title={taxApplied
                ? `Taxa de ${taxPercent}% já aplicada. Anterior: ${previousValue} RC`
                : negotiatedToOtherUser
                  ? `Aplicar desconto de ${taxPercent}% (personagem negociado: sem o desconto fixo de ${OWNED_SALE_FLAT_FEE_RC} RC)`
                  : `Aplicar desconto de ${taxPercent}% + ${OWNED_SALE_FLAT_FEE_RC} RC`}
            >
              <Percent size={12} />
              {taxApplied ? "✓ Taxa Aplicada" : negotiatedToOtherUser ? `Aplicar Taxa (-${taxPercent}%)` : `Aplicar Taxa (-${taxPercent}% -${OWNED_SALE_FLAT_FEE_RC} RC)`}
            </button>
          </div>
