...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Guard de reativação única (ref, imune a closures desatualizadas dos
  // effects): enquanto true, novos eventos de atividade NÃO reiniciam o
  // cronômetro do modal — sem ele, cada mousemove reagendava o setTimeout e
  // o modal "Reativando..." ficava na tela por tempo indeterminado.
  const idleRestoreInProgressRef = useRef(false);
  ...
  function enterIdleMode() {
    clearIdleTimer();
    idleRestoreInProgressRef.current = false;
    setIsIdleMode(true);
  ...
  function restoreFromIdle() {
    ...
    // Reativação JÁ em andamento: ignora o evento de atividade repetido.
    // Não reagendar o timeout garante a duração visual FIXA de 1 segundo,
    // por mais que o usuário continue movendo o mouse/teclando.
    if (idleRestoreInProgressRef.current) return;
    if (!isIdleMode && !isIdleRestoring) {
      scheduleIdleTimer();
      return;
    }
    if (idleRestoreTimerRef.current) window.clearTimeout(idleRestoreTimerRef.current);
    idleRestoreInProgressRef.current = true;
    // Reativação REAL imediata: isIdleMode=false religa os listeners/timers
    // pausados no mesmo render; o modal (isIdleRestoring) é só feedback.
