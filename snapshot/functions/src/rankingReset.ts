...
import {
  ...
  buildMonthlyTopPayload,
  getPreviousMonthKey,
  getUtcMonthKey,
  listMonthsToArchive,
  monthlyDocNeedsRewrite,
  type MonthlyRankingDoc,
...
export const scheduledRankingReset = onSchedule(
  ...
  async () => {
    ...
    // Projeção do quadro em DOC ÚNICO: coleta o estado DESEJADO de cada
    // usuário do mês (reconciliado a partir de dailyStats — a mesma base das
    // escritas da subcoleção) para regravar `rankingMonthly/{mês}.entries`.
    const desiredMonthlyDocs: MonthlyRankingDoc[] = [];
    statsDocs.forEach(({ uid, data }) => {
      ...
      desiredMonthlyDocs.push(desired);
    ...
    // Doc pai SEMPRE reconciliado com o quadro completo embutido (entries):
    // é a fonte de 1 leitura do painel e se autocorrige diariamente mesmo
    // quando nenhum doc da subcoleção mudou (ex.: projeção do fluxo de tempo
    // real falhou ontem). Escrita idempotente e barata (1 doc/dia).
    // createdAt só na primeira escrita (1 leitura para preservá-lo).
    const monthDocSnap = await monthDocRef.get();
    const top = buildMonthlyTopPayload(desiredMonthlyDocs);
    ...
    if (!monthDocSnap.exists || !record(monthDocSnap.data()).createdAt) {
      parentPayload.createdAt = FieldValue.serverTimestamp();
    }
    writeBatch.set(monthDocRef, parentPayload, { merge: true });
