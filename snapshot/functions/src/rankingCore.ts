...
// ─────────────────────────────────────────────────────────────────────────────
// TOP MENSAL CONSOLIDADO — doc único `rankingMonthly/{YYYY-MM}` (campo entries)
// ─────────────────────────────────────────────────────────────────────────────
//
// O quadro "Mês Atual" inteiro embutido no DOC PAI do mês, no mesmo padrão do
// `rankingHistory/{YYYY-MM}` (que já guarda o mês fechado num doc único): o
// painel carrega tudo com 1 LEITURA em vez de 1 leitura por usuário da
// subcoleção. A subcoleção `users/{uid}` continua sendo a FONTE (transação de
// stats + reconciliação diária escrevem nela primeiro); o doc pai é uma
// PROJEÇÃO ordenada dela — e o frontend mantém fallback para a subcoleção
// enquanto este campo não existir (CFs antigas ainda deployadas).

/** Teto de entradas embutidas no doc pai (bem abaixo do limite de 1MB). */
export const MONTHLY_TOP_LIMIT = 300;

export interface MonthlyTopPayload {
  /** Quadro ordenado por score (desc), truncado em MONTHLY_TOP_LIMIT. */
  entries: MonthlyRankingDoc[];
  /** Total real de usuários com doc no mês (para detectar truncamento). */
  totalPlayers: number;
}

/**
 * Projeta o conteúdo da subcoleção do mês no payload do doc pai.
 * Ordenação idêntica à consulta que o painel fazia (score desc); empate
 * resolvido por uid para a projeção ser determinística (escritas comparáveis).
 */
export function buildMonthlyTopPayload(docs: MonthlyRankingDoc[]): MonthlyTopPayload {
  const sorted = [...docs].sort((a, b) => (b.score - a.score) || a.uid.localeCompare(b.uid));
  return {
    entries: sorted.slice(0, MONTHLY_TOP_LIMIT),
    totalPlayers: sorted.length,
  };
}

/** Normaliza um doc da subcoleção users/{uid} para o formato da projeção. */
export function monthlyDocFromRecord(uid: string, data: UnknownRecord): MonthlyRankingDoc {
  return {
    ...emptyMonthlyDoc(uid),
    legacy: data.legacy === true,
    score: num(data.score),
    concluidas: num(data.concluidas),
    totalParticipacoes: num(data.totalParticipacoes),
    totalMortes: num(data.totalMortes),
    totalDuracaoMs: num(data.totalDuracaoMs),
    totalPtsSoulwar: num(data.totalPtsSoulwar),
    totalPtsSanguine: num(data.totalPtsSanguine),
    ...
    ptsComMorte: num(data.ptsComMorte),
    services: num(data.services),
    sequenciaAtualSemMorte: num(data.sequenciaAtualSemMorte),
    maxSequenciaSemMorte: num(data.maxSequenciaSemMorte),
    totalRcDoadoAprovado: num(data.totalRcDoadoAprovado),
    servers: counterMap(data.servers),
    partners: counterMap(data.partners),
