...
import { applyPartyToMonthlyDoc, buildMonthlyTopPayload, monthlyDocFromRecord } from "./rankingCore.js";
...
async function commitPartyUserStats(party: LifecycleParty, eventId: string): Promise<void> {
  ...
  let committed = 0;
  // Meses cujo doc consolidado mudou nesta materialização — a projeção do
  // doc pai (quadro em doc único) é reescrita UMA vez por mês afetado, após
  // os commits individuais (fora das transações: a projeção é derivada e a
  // reconciliação diária corrige qualquer corrida residual).
  ...
  // Projeção do quadro mensal em DOC ÚNICO (rankingMonthly/{mês}.entries):
  // relê a subcoleção do(s) mês(es) alterado(s) e regrava o doc pai ordenado.
  // 1 leitura de subcoleção + 1 escrita por mês afetado e por PT concluída —
  // o painel passa a carregar o quadro inteiro com 1 leitura. Falha aqui não
  // invalida os stats (a reconciliação diária reescreve a projeção).
  for (const monthKey of monthsTouched) {
    try {
      const monthDocRef = firestore.collection(RANKING_MONTHLY_COLLECTION).doc(monthKey);
      const usersSnap = await monthDocRef.collection(MONTHLY_USERS_SUBCOLLECTION).get();
      const monthlyDocs = usersSnap.docs.map(docSnap => monthlyDocFromRecord(docSnap.id, record(docSnap.data())));
      const top = buildMonthlyTopPayload(monthlyDocs);
      await monthDocRef.set({
        monthKey,
        entries: top.entries,
        totalPlayers: top.totalPlayers,
        entriesUpdatedAt: FieldValue.serverTimestamp(),
        entriesUpdatedAtMs: Date.now(),
      }, { merge: true });
    } catch (err) {
      logger.error("monthly top projection failed", { eventId, partyId: party.id, monthKey, err: String(err) });
    }
  }
